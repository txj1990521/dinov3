...
class SearchWorker(QThread):
    ...
    def __init__(self, query_path, processor, model, device, index, all_paths, top_k):
        super().__init__()
...
class FabricSearchApp(QMainWindow):
    ...
    def init_backend(self):
        ...
        try:
            ...
            self.status_label.setText(f"✅ 就绪 | 库容量: {len(self.all_paths)} 张 | 设备: {self.device.upper()}")
            self.progress_bar.setVisible(False)
    ...
    def run_search(self, image_path):
        # 更新左侧预览
        pixmap = QPixmap(image_path).scaled(280, 200, Qt.KeepAspectRatio,
                                            Qt.SmoothTransformation)
